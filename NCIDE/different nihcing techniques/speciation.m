function [ niches,stagList,bestfit,niche_erase,pop_erase,sto_erase,record,sto,sto_fit,archive,archive_fit,uFList,uCRList] = speciation( x,costX,stagList,bestfit,sto,sto_fit,archive,archive_fit,uFList,uCRList)
T=30;
%% speciation
flag=[];
sn=1;
...
NP=size(x,1);
for i=1:NP/M
    [best,bestidx]=max(tempCostX);
    dis=zeros(1,NP);
    for j=1:NP
        dis(j)=norm(x(bestidx,:)-x(j,:));
    end
    dis(flag)=inf;
    flag=[flag,bestidx];
    [dis,idx]=sort(dis);
